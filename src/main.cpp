...
#if defined(NDEBUG)
# error "Wajacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc7c5fcb57a2415f9b6f178f1d77bf04f788d437bd2d19dacbc4fbe51cc4169fe");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wajacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wajacoin Signed Message:\n";
...
{
    // Wajacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // Wajacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Wajacoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // Wajacoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wajacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wajacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe0485f4689aa48df6b517ec58d5dbb8f8cca87c2ce0210c540d886a5b285a3a2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Boris Johnson is pushing forward for BREXIT deal, 29 Oct/2019/";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0457c09d489633008fa42587a14aaf131c1359165f853b66c9d20cd773328961609ee48cc0b820c1a15f610dbc14284fdf075496291e68a1b8224685379bc205a4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1572342404;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085235156;

        if (fTestNet)
        {
            block.nTime    = 1572342385;
            block.nNonce   = 385498437;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe6545ea31f897c2abb046880cfb1fd6154dc49bac417d395e44cf660de9b7d74"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc4, 0xbc, 0xd7 }; // Wajacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WajacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WajacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WajacoinMiner(CWallet *pwallet)
{
    printf("WajacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wajacoin-miner");
    ...
    try { loop {
        ...
        printf("Running WajacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WajacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WajacoinMiner, pwallet));
}
